node-template --telemetry-url ws://telemetry.polkadot.io:1024  \
	      ...
	      --alice --base-path /tmp/alice > logs/substrate_alice.log 2>&1 &

ALICE=/ip4/127.0.0.1/tcp/30333/p2p/QmRpheLN4JWdAnY7HGJfWFNbfkQCb6tFf4vvA6hgjMZKrR

node-template --telemetry-url ws://telemetry.polkadot.io:1024 \
              --chain=local  --validator --bootnodes $ALICE \
              --bob --base-path /tmp/bob --port 30334 > logs/substrate_bob.log 2>&1 &

exit
node-template --telemetry-url ws://telemetry.polkadot.io:1024 \
              --chain=local  --validator --bootnodes $ALICE \
              --charlie --base-path /tmp/charlie --port 30335 > logs/substrate_charlie.log 2>&1 &

node-template --telemetry-url ws://telemetry.polkadot.io:1024 \
              --chain=local  --validator --bootnodes $ALICE \
              --dave --base-path /tmp/dave --port 30336 > logs/substrate_dave.log 2>&1 &
